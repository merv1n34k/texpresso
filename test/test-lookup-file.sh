#!/bin/bash
# Test lookup-file: engine notifies about a failed file lookup (non-blocking),
...
# Wait for lookup-file for the target file (ignore .aux etc.)
while ! grep -q "lookup-file read failed \"$TARGET\"" "$OUTFILE" 2>/dev/null; do
  sleep 0.5
  if ! kill -0 "$PID" 2>/dev/null; then
    echo "FAIL: texpresso exited before emitting lookup-file for $TARGET"
...
echo "Got lookup-file for: $TARGET"
