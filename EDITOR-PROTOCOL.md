...
Pre-register a filename that the editor will provide later via `open`. When the engine tries to read this file and it is not yet available, the driver pauses the engine and emits `lookup-file read failed`. Once the editor sends `open` for the path, the engine resumes without restarting.
